...
| WCAG-correct initials contrast | `chattr-3.1.0-p21` | The initials-circle letter picked its colour with `computeLuminance() > 0.5`, but the black/white crossover is at ≈**0.179** — on mid-luminance hues (yellow/green) the old rule chose white at ~1.9:1, unreadable and below the 3:1 AA-large floor. Now compares both WCAG contrast ratios and takes the winner. Adds static `ProfileImageWidget.fallbackForegroundResolver` so the host app can own a single contrast rule shared by in-package and app-side avatars. |
| optional reaction-pill avatars | `chattr-3.1.0-p22` | `MessageReactionConfiguration.showReactedUserAvatars` (default `true` = upstream). The pill always listed one profile circle per reacting user — useful in a group, noise in a 1:1 where there are only two possible reactors and every single 👍 grew a circle next to it. With the flag off the pill shows a bare count instead, and only when more than one person reacted. |
| centred system-message notices | `chattr-3.1.0-p23` | `ChatBubbleConfiguration.isCenteredMessage` (`bool Function(Message)?`). Apps model system messages as coming from a reserved sender id, which the package lays out like any incoming bubble: indented by the avatar slot (so a "centred" notice sits off-centre, by a *varying* amount because the slot collapses on the last message of a group), wrapped in `SwipeToReply` (a notice can be swiped into a reply the app cannot send, so the send silently does nothing), and carrying a sender name, reply quote and edited marker it never has. Returning `true` lays the bubble out full-width and centred without any of that, and without the receipt. Long-press, the p18 highlight and reactions are untouched. |
| app-owned reaction sheet | `chattr-3.1.0-p24` | `MessageReactionConfiguration.onReactionPillTap` (`void Function(String messageId)?`) + optional `ReactionWidget.messageId`. The pill tap was hardwired to the package's `ReactionsBottomSheet`, which builds from an immutable `Reaction` (a row removed while it is open stays on screen) and whose only hook, `reactedUserCallback`, fires per row **without a message id** — so an app that wants to remove the viewer's own reaction would have to guess which message a (user, emoji) pair came from. With the callback set the package hands the tap over, id included; null keeps upstream behaviour. |
| stable avatar cache key | `chattr-3.1.0-p25` | Static `ProfileImageWidget.cacheKeyResolver` (`String? Function(String url)?`), same shape as the p20/p21 resolvers. `CachedNetworkImage` keys its disk cache on the URL; with **signed** avatar URLs every refreshed signature reads as a new image, so the same picture is re-downloaded and its cache files grow without bound. Return a stable identity (the storage path); null keeps keying on the URL. |
| dimmable jump-to-message glow | `chattr-3.1.0-p26` | `RepliedMsgAutoScrollConfig.highlightGlowSpread` (default **2**, was a hardcoded 8) and `highlightGlowBlur` (default 28). The p18/p19 pulse stacked two shadows at `spreadRadius` 4 and 8. `spreadRadius` inflates the shadow rectangle *before* it is blurred, so it adds to the blur instead of softening into it — at 8 + a 28px blur the glow reached further than the gap between two bubbles and lit up the neighbours, reading as "somewhere around here" rather than "this one". Blur carries the softness and spread carries the size, so only the latter came down; both are now the host app's to set. |
